...
EFI_STATUS pdUpdateState() {
    ...
    if (!EFI_ERROR(Status)) {
        ...
        if (SPointerState.RelativeMovementX != 0 || SPointerState.RelativeMovementY != 0) {
            ...
            INTN tempRelX = (INTN)SPointerState.RelativeMovementX;
            tempRelX = (tempRelX * (INTN)UGAWidth) / (INTN)MouseResX; // Scale by screen resolution
            tempRelX = (tempRelX * (INTN)GlobalConfig.MouseSpeed) >> 10; // Apply mouse speed and final shift

            ScreenRelX = tempRelX;

            INTN tempRelY = (INTN)SPointerState.RelativeMovementY;
            tempRelY = (tempRelY * (INTN)UGAHeight) / (INTN)MouseResY; // Scale by screen resolution
            tempRelY = (tempRelY * (INTN)GlobalConfig.MouseSpeed) >> 10; // Apply mouse speed and final shift

            ScreenRelY = tempRelY;

            // Pre-check for Y-axis overflow before addition
            INTN newY = State.Y + ScreenRelY;
            if (ScreenRelY > 0 && newY < State.Y) { // Overflowed positively
                State.Y = 599; // Clamp to max hardcoded Y
            } else if (ScreenRelY < 0 && newY > State.Y) { // Overflowed negatively
                State.Y = 5; // Clamp to min hardcoded Y
            } else {
                State.Y = newY; // No overflow, proceed with addition
            }

            // Pre-check for X-axis overflow before addition
            INTN newX = State.X + ScreenRelX;
            if (ScreenRelX > 0 && newX < State.X) { // Overflowed positively
                State.X = 795; // Clamp to max hardcoded X
            } else if (ScreenRelX < 0 && newX > State.X) { // Overflowed negatively
                State.X = 5; // Clamp to min hardcoded X
            } else {
                State.X = newX; // No overflow, proceed with addition
            }
